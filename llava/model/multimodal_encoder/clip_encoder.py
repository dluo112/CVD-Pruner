...
####这是为了复现holov才做的改动  如果要复原跑visionzip与ours 请用上面的
#     def feature_select(self, image_forward_outs):
#         select_feature_type = self.select_feature
...
#             select_layers = [-2, -5, -8, -11, 6]
#             image_features = torch.cat(
#                 [image_forward_outs.hidden_states[i] for i in select_layers],
#                 dim=-1,
#             )
#             image_attentions = image_forward_outs.attentions[self.select_layer]
#             select_feature_type = select_feature_type.replace("slice_m25811_f6_", "")
...
class CLIPVisionTowerS2(CLIPVisionTower):
    ...
    def forward_feature(self, images):
        image_forward_outs = self.vision_tower(images.to(device=self.device, dtype=self.dtype), output_hidden_states=True)
        image_features, image_attentions = self.feature_select(image_forward_outs).to(images.dtype)
        return image_features
